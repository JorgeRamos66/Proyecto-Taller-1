<section class="comercializacion text-justify container">
  <div style="text-align: center;"><h1 class="my-2 badge" style="text-shadow: none;color: blue;backdrop-filter: blur(10px); background-color: rgb(255, 255, 255, 0.2);">Opciones de Comercialización</h1></div>
  
  <div class="row">
      <div class="col-lg-6">
          <div class="entrega rounded-2 my-2 text-justify text-wrap container" style="backdrop-filter: blur(10px); background-color: rgb(255, 255, 255, 0.2);">
              <h3>Tipos de Entrega</h3>
              <ul>
                  <li>
                      <h4><i class="fas fa-store"></i> Retiro en Local</h4>
                      <p>Disponible en todas nuestras sucursales. Consulta horarios y direcciones.</p>
                  </li>
                  <li>
                      <h4><i class="fas fa-shipping-fast"></i> Envío a Domicilio</h4>
                      <p>Recibe tus productos en la comodidad de tu hogar. Elige entre correo o motomandado.</p>
                  </li>
              </ul>
          </div>
      </div>
      <div class="col-lg-6">
          <div class="envio rounded-2 my-2 text-justify text-wrap container" style="backdrop-filter: blur(10px); background-color: rgb(255, 255, 255, 0.2);">
              <h3>Formas de Envío</h3>
              <ul>
                  <li>
                      <h4><i class="fas fa-envelope"></i> Correo</h4>
                      <p>Envío estándar con cobertura nacional. Plazos de entrega según destino.</p>
                  </li>
                  <li>
                      <h4><i class="fas fa-motorcycle"></i> Motomandado</h4>
                      <p>Envío rápido y seguro en áreas locales. Ideal para entregas urgentes.</p>
                  </li>
              </ul>
          </div>
      </div>
  </div>
  <div class="row">
      <div class="col-lg-6">
          <div class="pago rounded-2 my-2 text-justify text-wrap container" style="backdrop-filter: blur(10px); background-color: rgb(255, 255, 255, 0.2);">
              <h3>Formas de Pago</h3>
              <ul>
                  <li>
                      <h4><i class="fas fa-credit-card"></i> Tarjeta de Crédito/Débito</h4>
                      <p>Paga en línea con total seguridad. Aceptamos las principales tarjetas.</p>
                  </li>
                  <li>
                      <h4><i class="fas fa-university"></i> Transferencia Bancaria</h4>
                      <p>Realiza tu pago mediante transferencia a nuestra cuenta bancaria. Consulta datos.</p>
                  </li>
              </ul>
          </div>
      </div>
      <div class="col-lg-6">
          <div class="informacion rounded-2 my-2 text-justify text-wrap container" style="backdrop-filter: blur(10px); background-color: rgb(255, 255, 255, 0.2);">
              <h3>Información Adicional</h3>
              <ul>
                  <li><i class="fas fa-exchange-alt"></i> Cambios y devoluciones: Consulta nuestra política de cambios y devoluciones.</li>
                  <li><i class="fas fa-tags"></i> Promociones y descuentos: Aprovecha nuestras ofertas especiales y descuentos.</li>
                  <li><i class="fas fa-question-circle"></i> Preguntas frecuentes: Encuentra respuestas a tus dudas en nuestra sección de preguntas frecuentes.</li>
              </ul>
          </div>
      </div>
  </div>
</section>