<div class="container my-0 mb-5 d-flex justify-content-center">
    <div class="card registro" style="border-color: green; color: white; text-shadow: none; backdrop-filter: blur(10px); background-color: rgb(255, 255, 255, 0.5);">
        <div class="card-header text-center">
            <h2 style="color: green;"><?= isset($categoria) ? 'Editar Categoría' : 'Alta Categoría'; ?></h2>
        </div>

        <?php $validation = \Config\Services::validation(); ?>

        <form method="post" action="<?= base_url('actualizar-categoria/' . (isset($categoria) ? $categoria['id_categoria'] : '')) ?>">
            ...
            <div class="card-body justify-content-center">
                ...
                <div class="form-floating">
                    <input name="descripcion_categoria" type="text" class="form-control" value="<?= isset($categoria) ? esc($categoria['descripcion_categoria']) : (isset($_POST['descripcion_categoria']) ? esc($_POST['descripcion_categoria']) : ''); ?>" placeholder="Ingrese la descripción de la categoría">
                ...
                <div class="mb-3">
                    <div class="form-check">
                        <input name="activo_categoria" type="checkbox" class="form-check-input" id="activo_categoria" value="SI" 
                               <?= isset($categoria) ? ($categoria['activo_categoria'] == 'SI' ? 'checked' : '') : 'checked'; ?>>
                ...
                <div class="container text-center">
                    <input type="submit" value="<?= isset($categoria) ? 'Actualizar' : 'Enviar'; ?>" class="btn btn-primary">
                    <a href="<?= base_url('gestion_categorias'); ?>" class="btn btn-danger">Atrás</a>
...
<script>
    function borrarCampos() {
        ...
        document.querySelector('input[name="activo_categoria"]').checked = true; // Marcado por defecto
